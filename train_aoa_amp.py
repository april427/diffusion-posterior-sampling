"""
Training script for diffusion model on AoA/Amplitude data
"""
...
from data.aoa_amp_dataset import AoAAmpDataset  # Import to register the dataset
...
def compute_loss(model, diffusion, batch, device):
    """Compute diffusion training loss using a simpler approach"""
...
def validate(model, diffusion, val_dataloader, device):
    """Validation loop"""
    model.eval()
    total_loss = 0
    num_batches = 0
...
def sample_and_save(model, diffusion, device, save_path, num_samples=4, data_channels=2, image_size=64):
    ...
    with torch.no_grad():
        ...
        for i in reversed(range(diffusion.num_timesteps)):
            ...
            with torch.no_grad():
                ...
                # Simple denoising step (this is a simplified version)
                if i > 0:
                    # Add some noise for non-final steps
                    noise = torch.randn_like(img)
                    coef1 = extract_and_expand(diffusion.sqrt_alphas_cumprod, t, img)
                    coef2 = extract_and_expand(diffusion.sqrt_one_minus_alphas_cumprod, t, img)
                    
                    # Simplified denoising (this is not the exact DDPM sampling)
                    img = img - 0.1 * predicted_noise + 0.01 * noise
                else:
                    # Final step
                    img = img - 0.1 * predicted_noise
...
def main():
    parser = argparse.ArgumentParser(description="Train diffusion model on AoA/Amplitude data")
    parser.add_argument('--model_config', type=str, default='configs/aoa_amp_config.yaml',
                       help='Path to model configuration file')
    parser.add_argument('--diffusion_config', type=str, default='configs/diffusion_config.yaml',
                       help='Path to diffusion configuration file')
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints/aoa_amp',
                       help='Directory to save checkpoints')
    parser.add_argument('--resume', type=str, default=None,
                       help='Path to checkpoint to resume from')
    parser.add_argument('--gpu', type=int, default=0,
                       help='GPU device to use')
    parser.add_argument('--log_dir', type=str, default='./logs/aoa_amp',
    ...
    model_params = {k: v for k, v in model_config.items() 
                   if k not in ['batch_size', 'learning_rate', 'num_epochs', 'save_interval', 'log_interval', 'dataset', 'dataloader', 'data_channels']}
    
    # Get data channels from config
    data_channels = model_config.get('data_channels', 3)  # Default to 3 for RGB
